        ...
        try:
            ...
            async with stream:
                async for chunk in stream:
                    # Some OpenAI-compatible providers (e.g. GigaChat SSE) omit ``id`` on stream
                    # chunks; LiveKit's ``ChatChunk`` requires a string.
                    chunk_id = chunk.id if chunk.id else ""
                    for choice in chunk.choices:
                        chat_chunk = self._parse_choice(chunk_id, choice, thinking)
