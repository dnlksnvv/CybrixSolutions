...
def _clip_preemptive_log(text: str, max_len: int = 200) -> str:
    t = " ".join(text.split())
    if len(t) <= max_len:
        return t
    return t[: max_len - 1] + "…"
...
def _preemptive_transcript_match(a: str | None, b: str | None) -> bool:
    """Equality for reusing preemptive SpeechHandle.

    If ``PREEMPTIVE_TRANSCRIPT_NORMALIZE`` is set: lowercase, remove ``.,!?;:``,
    then drop all whitespace (Salute partial vs final often differs by punctuation).

    Set ``PREEMPTIVE_TRANSCRIPT_MATCH_LOG=1`` for INFO logs of raw vs normalized strings.
    """
    ...
    use_norm = _env_flag("PREEMPTIVE_TRANSCRIPT_NORMALIZE")
    if use_norm:
    ...
    if _env_flag("PREEMPTIVE_TRANSCRIPT_MATCH_LOG"):
        logger.info(
            "preemptive transcript compare: preemptive_raw=%r final_raw=%r "
            "PREEMPTIVE_TRANSCRIPT_NORMALIZE=%s compared_as=%r vs %r match=%s",
            _clip_preemptive_log(sa),
            _clip_preemptive_log(sb),
            use_norm,
            _clip_preemptive_log(na),
            _clip_preemptive_log(nb),
            result,
        )
...
class AgentActivity(RecognitionHooks):
    ...
    def on_preemptive_generation(self, info: _PreemptiveGenerationInfo) -> None:
        ...
        hook_log = _env_flag("PREEMPTIVE_GENERATION_HOOK_LOG")
        ts_wall = time.time()
        ts_mono = time.perf_counter()
        tx = _clip_preemptive_log(info.new_transcript or "")

        def _hook_skip(reason: str, **details: Any) -> None:
            if not hook_log:
                return
            tail = " ".join(f"{k}={v!r}" for k, v in details.items())
            logger.info(
                "preemptive hook SKIP wall_ts=%.3f mono_ts=%.3f reason=%s transcript=%r %s",
                ts_wall,
                ts_mono,
                reason,
                tx,
        ...
        if not preemptive_opts["enabled"]:
            _hook_skip("preemptive_disabled_in_session_options")
            return
        if self._scheduling_paused:
            _hook_skip("scheduling_paused")
            return
        if self._new_turns_blocked:
            _hook_skip("new_turns_blocked")
            return
        if self._current_speech is not None and not self._current_speech.interrupted:
            _hook_skip(
                ...
                speech_done=self._current_speech.done(),
                allow_interruptions=self._current_speech.allow_interruptions,
            )
            return
        if not isinstance(self.llm, llm.LLM):
            _hook_skip("llm_not_pipeline_llm", llm_type=type(self.llm).__name__)
            return

        # Same normalized transcript as the active preemptive: keep the existing LLM+TTS
        # pipeline and do not burn max_retries (duplicate Salute prefights, etc.).
        if self._preemptive_generation is not None and _preemptive_transcript_match(
            self._preemptive_generation.info.new_transcript,
            info.new_transcript,
        ):
            _hook_skip(
                "unchanged_preemptive_transcript",
                speech_id=self._preemptive_generation.speech_handle.id,
            )
            return
        ...
        if info.started_speaking_at is not None:
            spoke_s = ts_wall - info.started_speaking_at
            if spoke_s > preemptive_opts["max_speech_duration"]:
                _hook_skip(
                    "max_speech_duration_exceeded",
                    spoke_s=round(spoke_s, 3),
                    max_speech_duration=preemptive_opts["max_speech_duration"],
                )
                return
        ...
        if self._preemptive_generation_count >= preemptive_opts["max_retries"]:
            _hook_skip(
                "max_retries_reached",
                preemptive_generation_count=self._preemptive_generation_count,
                max_retries=preemptive_opts["max_retries"],
            )
            return
        ...
        if hook_log:
            logger.info(
                "preemptive hook START wall_ts=%.3f mono_ts=%.3f attempt=%s max_retries=%s "
                "transcript=%r speech_id=%s preemptive_tts=%s",
                time.time(),
                time.perf_counter(),
                self._preemptive_generation_count,
                preemptive_opts["max_retries"],
                tx,
                speech_handle.id,
                preemptive_opts.get("preemptive_tts"),
            )
    ...
    async def _user_turn_completed_task(
        ...
        if preemptive := self._preemptive_generation:
            ...
            transcript_ok = _preemptive_transcript_match(
                preemptive.info.new_transcript, user_message.text_content
            )
            chat_ok = preemptive.chat_ctx.is_equivalent(temp_mutable_chat_ctx)
            tools_ok = preemptive.tools == self.tools
            tool_choice_ok = preemptive.tool_choice == self._tool_choice
            if transcript_ok and chat_ok and tools_ok and tool_choice_ok:
                ...
                if _env_flag("PREEMPTIVE_TRANSCRIPT_MATCH_LOG"):
                    logger.info(
                        "preemptive reuse: ok (transcript_match=%s chat_ctx=%s tools=%s tool_choice=%s)",
                        transcript_ok,
                        chat_ok,
                        tools_ok,
                        tool_choice_ok,
                ...
                if _env_flag("PREEMPTIVE_TRANSCRIPT_MATCH_LOG"):
                    logger.info(
                        "preemptive reuse: skipped transcript_match=%s chat_ctx=%s tools=%s "
                        "tool_choice=%s",
                        transcript_ok,
                        chat_ok,
                        tools_ok,
